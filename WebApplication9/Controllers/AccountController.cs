    ...
    public class AccountController : Controller
    {
        ...
        public void SendEmailConfirmation(ApplicationUser user)
        {
            ...
            m.Body = string.Format("Для завершения регистрации перейдите по ссылке:" +
                ...
                Url.Action("ConfirmEmail", "Account", new { Token = user.Id, Email = user.UserName }, Request.Url.Scheme));
            m.IsBodyHtml = true;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var identityNameClaim = new IdentityUserClaim { ClaimType = "Name", ClaimValue = model.Name };
                    var identitySurNameClaim = new IdentityUserClaim { ClaimType = "Surname", ClaimValue = model.SurName };
                    var identityPhoneClaim = new IdentityUserClaim { ClaimType = "MobilePhone", ClaimValue = model.Phone };
                    user.Claims.Add(identityNameClaim);
                    user.Claims.Add(identitySurNameClaim);
//=======
//                    var identityNameClaim = new IdentityUserClaim { ClaimType = "Name", ClaimValue = model.Name };
//                    var identitySurNameClaim = new IdentityUserClaim { ClaimType = "Surname", ClaimValue = model.SurName };
//                    var identityEmailClaim = new IdentityUserClaim { ClaimType = "Email", ClaimValue = model.Email };
//                    var identityPhoneClaim = new IdentityUserClaim { ClaimType = "MobilePhone", ClaimValue = model.Phone };
//                    user.Claims.Add(identityNameClaim);
//                    user.Claims.Add(identitySurNameClaim);
//                    user.Claims.Add(identityEmailClaim);
//>>>>>>> origin/master
                    user.Claims.Add(identityPhoneClaim);
        ...
        public async Task<ActionResult> ConfirmEmail(string Token, string Email)
        {
            ...
            if (user != null)
            {
                if (user.UserName == Email)
                {
                    ...
                    await SignInAsync(user, isPersistent: false);
                    return RedirectToAction("Index", "Home", new { ConfirmedEmail = user.UserName });
                }
                else
                {
                    return RedirectToAction("Confirm", "Account", new { Email = user.UserName });
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.RemoveLoginSuccess ? "Внешние средства авторизации откючены."
        ...
        //
        // GET: /Account/ManageData
        public ActionResult ManageUserData()
        {
            //String id = User.Identity.GetUserId();
            //AspNetUsers user = _entities.AspNetUsers.Find(id);
            //ManageUserData 
            return View();
        }
